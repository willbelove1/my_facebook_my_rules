 ...
 * Mục đích: Quản lý giao diện người dùng, sử dụng ctx.saveSettings (async) để lưu.
 * Phiên bản: 2.1.0 (Sử dụng async saveSettings từ context)
 */
(function() {
  "use strict";
  ...
  FBCMF.registerModule("UIManager", async (ctx) => {
    ...
    const lightThemeCSS = `
      ...
      #fbcmf-settings-popup textarea { /* Added textarea */
       ...
       #fbcmf-settings-popup select {
         background-image: url("data:image/svg+xml,%3csvg xmlns=\'http://www.w3.org/2000/svg\' viewBox=\'0 0 16 16\'%3e%3cpath fill=\'none\' stroke=\'%231c1e21\' stroke-linecap=\'round\' stroke-linejoin=\'round\' stroke-width=\'2\' d=\'M2 5l6 6 6-6\'/%3e%3c/svg%3e");
         background-repeat: no-repeat;
         background-position: right .7em top 50%;
         background-size: .8em auto;
         -webkit-appearance: none;
    ...
    async function saveSettingsHandler() {
      ...
      try {
        ...
        if (success) {
          ...
          showStatusMessage(getLabel("settingsSaved"), "success");
          ...
          if (newSettings.language !== (ctx.settings?.language || 'vi')) {
              // Re-render popup with new labels (optional, might be complex)
              // Or simply inform user reload is needed for language change
              log("Language changed, reload might be needed for full UI update.");
          }
    ...
    console.log("[UIManager] ✅ Đã sẵn sàng (sử dụng Context API).");
  });
})();
