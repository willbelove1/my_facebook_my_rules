  ...
  FBCMF.registerModule('FeedExpander', async (ctx) => {
    ...
    function expandNewsFeed() {
      ...
      feedItems.forEach(item => {
        // Sử dụng 99% thay vì 100% để tránh lỗi icon
        if (item.style.width !== '99%') {
          item.style.width = '99%';
      ...
      if (currentCount > 0) {
        log(`Đã mở rộng ${currentCount} mục news feed thành 99% (tổng: ${expandedCount})`);
      }
    }
    ...
    function restoreOriginalStyles() {
      ...
      // Fallback: khôi phục tất cả .x193iq5w có width 99% hoặc 100%
      const allFeedItems = document.querySelectorAll('.x193iq5w');
      allFeedItems.forEach(item => {
        if (item.style.width === '99%' || item.style.width === '100%') {
          item.style.width = '';
          restoredCount++;
        }
      });
    ...
    function expandNow() {
      if (isActive && isHomePage()) {
        expandNewsFeed();
        log('Đã chạy mở rộng news feed thành 99% theo yêu cầu');
      } else {
        log('FeedExpander không active hoặc không ở trang được phép, bỏ qua yêu cầu expandNow');
      }
    }
