 ...
 * Cải tiến: Sửa lỗi giật lag + Chỉ hoạt động ở trang chủ
 */
(function() {
  'use strict';
  ...
  FBCMF.registerModule('FeedExpander', async (ctx) => {
    ...
    function log(message, level = 'verbose') {
      if (ctx.settings?.verbosity === 'verbose' || level === 'error') {
        console[level === 'error' ? 'error' : 'log'](`[FeedExpander v4.2] ${message}`);
      }
    }
    ...
    // Áp dụng CSS để mở rộng feed với cải tiến chống giật lag
    function applyCSSExpansion() {
      ...
      styleElement.textContent = `
        /* CSS cải tiến chống giật lag - áp dụng transition mượt mà */
        ...
        /* Mở rộng story container với transition */
        ...
        /* Đồng bộ hóa tất cả feed items để tránh giật lag */
        ...
        /* Force consistent width cho tất cả posts */
        div.x193iq5w:not(.x1hnll1o) {
          width: 100% !important;
          max-width: 1200px !important;
          margin-left: auto !important;
          margin-right: auto !important;
      ...
      log('✅ Đã áp dụng CSS mở rộng news feed (anti-lag)');
    }
    ...
    function hideRightSidebarItems() {
      ...
      // Ẩn [role="complementary"] nếu chứa contacts chỉ ở trang chủ
    ...
    // Debounced function để xử lý DOM mutations - cải tiến chống lag
    function debouncedExpansion() {
      ...
      }, 100); // Giảm delay để responsive hơn
    }
    ...
    // Thiết lập MutationObserver với hiệu suất cao - cải tiến chỉ hoạt động ở trang chủ
    function setupOptimizedObserver() {
      if (!isHomePage() || !ctx.settings?.expandNewsFeed) {
        log('Không thiết lập observer: không ở trang chủ hoặc tính năng tắt');
        return;
      }

      // Quan sát toàn bộ body với debounce để tối ưu hiệu suất
      mutationObserver = new MutationObserver((mutations) => {
        // Chỉ xử lý khi đang ở trang chủ
        if (!isHomePage()) {
          return;
        }
    ...
    function init() {
      try {
        ...
        log('✅ FeedExpander v4.2 đã khởi tạo thành công (chỉ hoạt động ở trang chủ)');
